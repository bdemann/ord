...
use crate::{
  ...
  templates::ServerConfig, Index, InscriptionId,
};
...
impl Server {
  ...
  pub(super) async fn inscription_json_by_id(
    ...
    inscription::inscription_json_by_id(server_config, index, inscription_id).await
  }

  pub(super) async fn inscription_json_by_index(
    Extension(server_config): Extension<Arc<ServerConfig>>,
    Extension(index): Extension<Arc<Index>>,
    Path(DeserializeFromStr(inscription_index)): Path<DeserializeFromStr<i32>>,
  ) -> ServerResult<String> {
    inscription::inscription_json_by_index(server_config, index, inscription_index).await
  }
  ...
  pub(super) async fn outputs_for_block_by_hash(
    ...
    outputs::outputs_for_block_by_hash(server_config, index, block_hash).await
  }
  ...
  pub(super) async fn outputs_for_block(
    Extension(server_config): Extension<Arc<ServerConfig>>,
    Extension(index): Extension<Arc<Index>>,
    Path(DeserializeFromStr(block_index)): Path<DeserializeFromStr<u32>>,
  ) -> ServerResult<String> {
    outputs::outputs_for_block(server_config, index, block_index).await
  }
  ...
  pub(super) async fn inscriptions_for_block(
    ...
    inscription::inscriptions_for_block(server_config, index, block_index).await
  }
  ...
  pub(super) async fn inscriptions_for_block_by_hash(
    ...
    inscription::inscriptions_for_block_by_hash(server_config, index, block_hash).await
  }

  pub(super) async fn paginated_inscriptions_for_block(
    Extension(server_config): Extension<Arc<ServerConfig>>,
    Extension(index): Extension<Arc<Index>>,
    Path((DeserializeFromStr(block_index), DeserializeFromStr(start), DeserializeFromStr(count))): Path<(DeserializeFromStr<u32>, DeserializeFromStr<usize>, DeserializeFromStr<usize>)>,
  ) -> ServerResult<String> {
    inscription::paginated_inscriptions_for_block(server_config, index, block_index, start, count)
      .await
  }
  ...
  pub(super) async fn inscription_json(
    ...
    Path((DeserializeFromStr(start), DeserializeFromStr(end))): Path<(
      DeserializeFromStr<i32>,
      DeserializeFromStr<i32>,
    )>,
  ) -> ServerResult<String> {
    inscription::inscription_json(server_config, index, start, end).await
  }
